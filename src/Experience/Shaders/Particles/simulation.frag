...
void main() {
    ...
    // Wave Animation (State 6)
    if (uState1 == 6 || uState2 == 6) {
        float waveMix = 0.0;
        if (uState1 == 6 && uState2 == 6) waveMix = 1.0;
        else if (uState1 == 6) waveMix = 1.0 - uTransition;
        else waveMix = uTransition;

        if (waveMix > 0.0) {
            // Use currentTarget (stable) for noise coordinates
            float n = snoise(vec3(currentTarget.x * frequency, currentTarget.z * frequency, timer * 0.1));
            
            vec3 wavePos = currentTarget;
            wavePos.y += n * amplitude;
            
            currentTarget = mix(currentTarget, wavePos, waveMix);
        }
    }
    ...
    vec3 targetPos = mix(pos, currentTarget, 0.5 + currentChaos * 0.5);
